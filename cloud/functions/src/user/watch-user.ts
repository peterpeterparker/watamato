...
export async function watchUserCreate(
...
async function cloneFlats(userRecord: admin.auth.UserRecord) {
  ...
  try {
    ...
    const promises: Promise<string>[] = flats.map((flat: Flat) => {
    ...
    const flatIds: string[] = await Promise.all(promises);

    await createUser(userRecord, flatIds);
  } catch (err) {
    console.error(err);
  }
}
