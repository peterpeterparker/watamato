...
export function createUser(userRecord: admin.auth.UserRecord): Promise<void> {
...
export function findAllUsers(): Promise<User[] | undefined> {
    return new Promise<User[] | undefined>(async (resolve, reject) => {
        try {
            const collectionRef: admin.firestore.CollectionReference = admin.firestore().collection('/users/');
