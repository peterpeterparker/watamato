...
export class UserService {
    ...
    init() {
        this.authService.user().pipe(filter(user => user !== undefined), take(1)).subscribe(async (user: FirebaseUser) => {
            try {
    ...
    private load(user: FirebaseUser): Promise<void> {
        return new Promise<void>((resolve, reject) => {
            ...
            this.subscription = doc.valueChanges().subscribe((data: UserData) => {
